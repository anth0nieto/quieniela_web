...
<body>
	...
	<div style="width:100%;" align="center">
		<p aling ="center"><font style="font-family: ArialDejaVu Sans; color:#30456B;"  size="5" ><br>Pronósticos de {{ $quiniela[0]->nombre }}<br><br></font></p>
	</div>
			...
			@foreach($jugadas as $jugada)	
				@if($jugada->id_user != $id_user)
					...
					<td colspan="4" align ="center" style="background-color:#5cb85c;">
						<b style="color:#fff;">Pronósticos de {{$user->username}}</b>
					</td>
					</tr>
					...
					<tr align="center">
						<td style="width:40%">{{$equipos[$count]->nom_local}}</td>
						<td style="width:10%">{{$jugada->goles_local}}</td>
						<td style="width:10%">{{$jugada->goles_visitante}}</td>
						<td style="width:40%">{{$equipos[$count]->nom_visitante}}</td>
					</tr>
